...
const Plans = () => {
  ...
  return (
    <div className={`min-h-screen font-[Arial] ${isDarkMode ? 'bg-[#0a0a0a] text-white' : 'bg-gray-100 text-gray-900'}`}>
              ...
              <div className="relative">
                <div className="flex items-center space-x-2 sm:space-x-6">
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`text-sm sm:text-lg font-medium ${isDarkMode ? 'text-white' : 'text-black'} truncate max-w-24 sm:max-w-none`}>
                      {userData.username}
                    </span>
                  </div>
                  ...
                  {userData.avatar ? (
                    <img
                      src={userData.avatar}
                      alt="User avatar"
                      className={`h-6 w-6 sm:h-8 sm:w-8 rounded-full ${isDarkMode ? ' ring-2 ring-[#00BFFF]/50' : ' ring-2 ring-white ring-opacity-20'}`}
                    />
                  ) : (
      ...
      <div className="fixed top-20 sm:top-3 left-4 sm:left-20 z-50">
        <button
          onClick={() => navigate('/dashboard')}
          className={`px-3 sm:px-4 py-2 font-medium text-sm sm:text-base transition-all duration-200 rounded-lg ${
            isDarkMode
      ...
      <div className="max-w-7xl mx-auto text-center pt-24 sm:pt-20 px-4 sm:px-6 lg:px-8">
        <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-[Arial] mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
          Choose Your Plan
        </h1>
        <p className={`text-base sm:text-lg mb-8 sm:mb-12 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Flexible pricing for every team and budget
        </p>

        <div className="flex flex-col sm:flex-row sm:flex-wrap lg:flex-nowrap justify-center gap-6 sm:gap-8">
          {plans.map((plan, index) => (
            <div
              key={index}
              onClick={() => setIsDropdownOpen(false)}
              className={`w-full sm:flex-1 sm:max-w-sm rounded-2xl shadow-lg p-6 sm:p-8 border relative transform transition-all duration-300 flex flex-col ${
                isDarkMode 
            ...
            >
              ...
              <button
                ...
                className={`w-full py-3 rounded-lg flex items-center justify-center gap-2 font-semibold text-sm sm:text-base transition-all duration-300 mt-auto ${
                  userData.plan === plan.planType
                    ? isDarkMode 
